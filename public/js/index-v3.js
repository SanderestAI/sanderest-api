 ...
 * @param {Blob} recorderAudioAsBlob - recorded audio as a Blob Object
 */
function talkToIA(recorderAudioAsBlob) {
    ...
    reader.onload = (e) => {
        ...
        xhr.onreadystatechange = function () {
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                window.thredId = responseJSON.thredId;

                let audioIaElement = document.getElementById('iaAudio');
                if (audioIaElement) {
                    audioIaElement.src = '/audios/assistances/3/' + responseJSON.lastRunId + '.mp3';
                    audioIaElement.play();
                }
            } else {
        ...
        data = new FormData()
        data.set('thredId',window.thredId)
        data.set('clipInput',base64URL)

        // Create and send a GET request
        // The first argument is the request type (GET, POST, PUT, DELETE, etc.)
        // The second argument is the endpoint URL
        xhr.open('POST', '/api/assistant/thred/getClientCommandFromMixAI', true);
        xhr.send(data);
    };
